...
contract DYNXTChef is Ownable, ReentrancyGuard {
    ...
    */
     ...
     */
    function withdraw(uint256 _amount, uint256 _lockId, uint256 _index) external nonReentrant {
        stakeTracker storage account = accounts[msg.sender][_lockId];
        LockInfo memory lock = lockInfo[_lockId];
    ...
    */
    function harvest(uint256 _lockId, uint256 _index) public {
        ...
        accounts[msg.sender][_lockId].rewards[_index] = 0;
        accounts[msg.sender][_lockId].claimedAt[_index] = block.number;
        _safeRewardTransfer(msg.sender, reward);
